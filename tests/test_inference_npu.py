...
def test_validate_cache_options_rejects_all_layers_without_method():
    with pytest.raises(ValueError, match="requires a cache method"):
        validate_cache_options(STANDARD_PIPELINE, False, False, True)


@pytest.mark.parametrize(
    ("enable_teacache", "enable_taylorseer"), [(True, False), (False, True)]
)
def test_validate_cache_options_rejects_turbo_caching(
    enable_teacache, enable_taylorseer
):
    with pytest.raises(ValueError, match="not supported by Turbo"):
        validate_cache_options(
            TURBO_PIPELINE, enable_teacache, enable_taylorseer, False
        )


def test_configure_cache_enables_teacache_for_all_layers():
    pipeline = SimpleNamespace(transformer=SimpleNamespace())

    configure_cache(pipeline, True, False, True, 0.08)

    assert pipeline.enable_taylorseer is False
    assert pipeline.transformer.enable_teacache is True
    assert pipeline.transformer.enable_teacache_for_all_layers is True
    assert pipeline.transformer.enable_taylorseer_for_all_layers is False
    assert pipeline.transformer.teacache_rel_l1_thresh == 0.08


def test_configure_cache_enables_taylorseer_for_all_layers():
    pipeline = SimpleNamespace(transformer=SimpleNamespace())

    configure_cache(pipeline, False, True, True, 0.05)

    assert pipeline.enable_taylorseer is True
    assert pipeline.transformer.enable_teacache is False
    assert pipeline.transformer.enable_teacache_for_all_layers is False
    assert pipeline.transformer.enable_taylorseer_for_all_layers is True
